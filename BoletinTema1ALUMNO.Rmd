...
distintos <- setdiff(rownames(a),rownames(b))
distintos
#Hay 4 animales que no están en ambos conjuntos: "Arctic fox" "Arctic ground squirrel" "Patas monkey" "Mole rat"   
...
df <- union(aux1,aux2)
df
#En este data frame se puede observar por ejemplo que el animal mas grande es el patas monkey, y el mas pequeño el mole rat
```
...
```{r}
library(car)
grafico1 <- qqPlot(log(a$brain))
grafico2 <- qqPlot(log(b$brain))
#Presentan un comportamiento similar, y siguen una distribución gaussiana 
```
...
libReturn<- library()
libReturn
#Nos devuelve todos los paquetes que hay en esta practica
head(libReturn,5)


#Hay 209 librerias instaladas (en este momento)
    ```
...
for (i in 1:nrow(r)){
  if (a[i]== TRUE){
    cuantosNaa <- cuantosNaa +1
  }
}
cuantosNaa #0 Nas en cult

cuantosNab <- 0
for (i in 1:nrow(r)){
  if (b[i]== TRUE){
    cuantosNab <- cuantosNab +1
  }
}
cuantosNab 
...
for (i in 1:nrow(r)){
  if (d[i]== TRUE){
    cuantosNad <- cuantosNad +1
  }
}
cuantosNad


#No hay NA en ningun lado

r<- Chile
str(r) #2700 registros de  8 variables
summary(r) #Hay 1 NA en age, 11 en education, 98 en incomes. 17 en statusquo y 168 en vote
...
grafico <- qqPlot(cabbages$HeadWt)
grafico <- qqPlot(cabbages$VitC)
#Lo verifican
```
...
hist(c39$VitC, breaks =20)
hist(c52$VitC, breaks =20)
#La c52 tiene mas vitamina C
```
...
#Diagrama de dispersión etiquetando según un factor.
# Planteamos 2 modelos lineales, uno para los datos de cada factor
Model1 <- lm(cabbages$Headwt ~ cabbages$VitC, data = cabbages )
Model2 <- lm(cabbages$Headwt ~ cabbages$VitC, data = cabbages)
# Representamos las rectas correspondientes a cada modelo lineal
abline(Model1, lty=2)
abline(Model2)
